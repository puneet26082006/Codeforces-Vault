...
void solve() {
    ...
    int posQ = 0 ;
    for(int i = 0 ; i < n ; i++){
        if (p[i] > p[posP]) posP = i;
        if (q[i] > q[posQ]) posQ = i;
        
        pair<int, int> cand1 = {max(p[posP], q[i - posP]), min(p[posP], q[i - posP])};
        pair<int, int> cand2 = {max(p[i - posQ], q[posQ]), min(p[i - posQ], q[posQ])};
        
        if (cand1 > cand2) {
            r[i] = (p1[posP] + q1[i - posP]) % mod;
        } else {
            r[i] = (p1[i - posQ] + q1[posQ]) % mod;
        }
    }
