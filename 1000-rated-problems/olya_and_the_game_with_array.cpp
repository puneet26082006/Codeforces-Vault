...
void solve() {
    ...
    while(n--){
        ...
        first_min.pb(first);
        second_min.pb(second);


    }

    ll global_min = *min_element(first_min.begin(), first_min.end());
    ll min_second = *min_element(second_min.begin(), second_min.end());

    ll sum = 0 ;
    for(auto &it : second_min) sum += it ;
