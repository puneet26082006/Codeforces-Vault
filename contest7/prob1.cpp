...
#include <algorithm>
#include <cmath>
#include <unordered_map> // Added this missing header!
...
void solve() {
    ...
    for(int i = first ; i <= last ; i++){
        bool iswall = (s[i] == '0') && (s[i - 1] == '0' || s[i + 1] == '0');
        
        if(iswall){
            if(current_len > 0){
                total_max += current_len ;
                total_min += (current_len / 2) + 1;
                current_len = 0 ;
            }
        } else {
            current_len++ ;
...
int main() {
    ...
    return 0; // Added return 0 for good measure
}